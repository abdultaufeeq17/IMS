<?php
	...
	if (isset($_POST['reg_user'])) {
		...
		if (empty($first_name)){array_push($errors, "Enter first name");}
		if (empty($last_name)){array_push($errors, "Enter last name");}
		if (empty($email)){array_push($errors, "Enter email id");}
		if (empty($password_1)){array_push($errors, "Create a password");}
		if (empty($password_2)){array_push($errors, "Confirm your password");}
		if (empty($mobile)){array_push($errors, "Enter mobile number");}
		if ($password_1 != $password_2){array_push($errors, "Those passwords didn’t match. Try again.");}
	...
	if(isset($_POST['submit'])) {
		...
		if (count($errors) == 0) {
			...
			if (md5($_POST['password']) !== $password) {echo "Wrong Password. Try again.";}
